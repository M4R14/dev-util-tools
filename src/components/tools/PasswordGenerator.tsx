...
import { RefreshCw, Check, ShieldCheck, ShieldAlert, Shield, Copy, Wand2 } from 'lucide-react';
        ...
        <div className="space-y-6 md:sticky md:top-6">
            ...
            <div className="p-4 rounded-xl border border-border bg-card space-y-3">
                 <div className="flex items-center justify-between">
                    <span className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Security Strength</span>
                    <span className={cn("text-xs font-bold uppercase flex items-center gap-1.5", strength.textColor)}>
                        {strength.label === 'Strong' ? <ShieldCheck className="w-4 h-4" /> : strength.label === 'Medium' ? <Shield className="w-4 h-4" /> : <ShieldAlert className="w-4 h-4" />}
                        {strength.label}
                    </span>
                 </div>
                 <div className="h-2.5 w-full bg-muted/50 rounded-full overflow-hidden">
                    <div 
                        className={cn("h-full transition-all duration-700 ease-out rounded-full", strength.color)}
            ...
            <Card className="border-border/60 shadow-sm">
                <CardContent className="p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        <label className="text-sm font-medium text-muted-foreground mb-1 block">Character Sets</label>
                        {[
                            { id: 'upper', label: 'Uppercase', example: 'ABC', checked: includeUpper, set: setIncludeUpper },
                            { id: 'lower', label: 'Lowercase', example: 'abc', checked: includeLower, set: setIncludeLower },
                            { id: 'numbers', label: 'Numbers', example: '123', checked: includeNumbers, set: setIncludeNumbers },
                            { id: 'symbols', label: 'Symbols', example: '!@#', checked: includeSymbols, set: setIncludeSymbols },
                        ].map((opt) => (
                            <div 
                                key={opt.id}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className={cn(
                                        ...
                                        opt.checked ? "bg-primary border-primary text-primary-foreground" : "border-muted-foreground/30 bg-transparent"
                                    )}>
                                        {opt.checked && <Check className="w-3 h-3" />}
                                    </div>
                                    <div className="space-y-0.5">
                                        <div className={cn("font-semibold text-sm", opt.checked ? "text-foreground" : "text-muted-foreground")}>{opt.label}</div>
                                    </div>
                                </div>
