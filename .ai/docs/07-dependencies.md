...
| `jwt-decode` | JWT base64url + JSON parsing. Deliberately not used on its own: it skips segment-count and claim-shape checks, so `src/lib/jwt.ts` keeps that validation. Verifies nothing. |
| `jose` | JWT signing and HMAC verification over Web Crypto (`src/lib/jwtSign.ts`). Splits into its own lazy chunk (~21 kB raw) loaded only by the two JWT tools; never reaches the eager bundle. Secure contexts only. |
